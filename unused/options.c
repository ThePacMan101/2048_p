...
void tickDeleteUser(tCommand command, bool *running, tGame *game, tState *state) {
    ...
    //Tem algum getc a mais, então quero passar algo aleatório para esse getc, e não mostrar na tela. 



    // printf("%c",command);
    char aux[20],aux2[20];              //  This is a workaround witchcraft so that
    scanf("%s", aux);
